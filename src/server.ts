...
function normalizeResourcePath(input: string): string {
  ...
  if (rootResources.has(relativePath)) {
    return `src/main/resources/${relativePath}`;
  }
...
function normalizeJavaVersion(input: string): string {
  ...
  return normalized === '17' ? '17' : '21';
}

function paperApiVersion(minecraftVersion: string): string {
...
function createTrustedPom(
  ...
  const safePluginName = sanitizeArtifactName(pluginName);
  const artifactId = safePluginName.toLowerCase();
...
async function writeTrustedProject(
  ...
  for (const file of request.files) {
    ...
    if (!resolvedTarget.startsWith(resolvedRoot)) {
      throw new Error(`File escaped build directory: ${relativePath}`);
    }
  ...
  const pluginYmlPath = path.join(resourcesDirectory, 'plugin.yml');
  const paperPluginYmlPath = path.join(
    resourcesDirectory,
    'paper-plugin.yml'
  );

  const hasPluginYml = await fs
    .access(pluginYmlPath)
    .then(() => true)
    .catch(() => false);

  const hasPaperPluginYml = await fs
    .access(paperPluginYmlPath)
    .then(() => true)
    .catch(() => false);

  if (!hasPluginYml && !hasPaperPluginYml) {
    throw new Error(
      'Project is missing plugin.yml or paper-plugin.yml. ' +
      'Generate one at src/main/resources/plugin.yml.'
    );
  }
  ...
  await fs.writeFile(
    ...
    createTrustedPom(pluginName, minecraftVersion, javaVersion),
    'utf8'
  );
}
...
async function runCommand(
  ...
  return await new Promise((resolve, reject) => {
    ...
    const appendLogs = (chunk: Buffer) => {
      if (logs.length < maximumLogCharacters) {
        logs += chunk
          .toString('utf8')
          .slice(0, maximumLogCharacters - logs.length);
      }
    };
  ...
  async (req, res) => {
    ...
    if (!parsed.success) {
      res.status(400).json({
        ...
        logs: JSON.stringify(parsed.error.issues, null, 2),
    ...
    try {
      const result = await buildLimiter(async () => {
        const request = parsed.data;
        const buildId = randomUUID();

        const temporaryDirectory = await fs.mkdtemp(
          path.join(os.tmpdir(), 'novx-build-')
        );
        ...
        try {
          ...
          const targetEntries = await fs.readdir(
            targetDirectory
          );
          ...
          const pluginName = sanitizeArtifactName(
            readOption(
              request.options,
              'pluginName',
              'NovxPlugin'
          ...
          await uploadArtifact(
            path.join(targetDirectory, jarName),
            jarRemotePath,
